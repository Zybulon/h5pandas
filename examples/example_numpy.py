...
The main difference is that h5pandas gives `pandas.DataFrame` objects instead of `h5py.Dataset` objects, even if the file has not been written with h5pandas.

h5pandas is fully compatible with h5py: you can write a dataset with one library and read with the other indifferently.

If you write a dataframe with h5pandas, columns names, index and attributes will be saved in the file and can be retrieve later with h5pandas.
...
# The function ndarray_to_hdf is the easiest way to write an array into a HDF5 with h5pandas
h5pd.ndarray_to_hdf(df0, "foo.h5", "foo", columns=columns, index=index)
