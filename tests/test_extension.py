...
def test_general_behavior():
    ...
    with h5py.File(
        ...
        b = result.b
        b.mean()
        print("Test groupby mean", time.time() - t0)

        df2 = pd.DataFrame(arr[:, 0:5], columns=["a", "b", "c", "d", "e"])
        result = df2.groupby("a", as_index=True)
        t0 = time.time()
        c = result.b
        c.mean()
        print("Test groupby mean classic dataFrame", time.time() - t0)
